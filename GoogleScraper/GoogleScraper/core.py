...
def getKeywords(config):
    ...
    return keywords, kwfile


def getSearchEngines(search_engines, supported_search_engines):
...
def main(return_results=False, parse_cmd_line=True, config_from_dict=None):
    ...
    keywords, kwfile = getKeywords(config)
    shellConfig(config)
    search_engines = getSearchEngines(config.get('search_engines', ['google']),
